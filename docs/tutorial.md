...
本课的作业是编写这个调查助手。写好后，把 AlexNet 论文的 PDF 交给它，检查它能否读取论文、检测本机环境，并据此生成调查报告。AlexNet 论文是课堂测试材料，助手也应能接收其他论文的 PDF。
...
先看论文助手。论文保存在本地，我们给助手提供 `read_paper` 工具：模型指定页码，工具提取这些页的文字，再把文字返回给模型。

读取函数已经写好，你只需实现工具调用和结果回传。

负责请求模型和执行工具的是 `Agent` 对象。它的 `model` 用来请求 DeepSeek，`system` 保存工作说明，`tools` 保存可用工具；调用 `run(任务说明)` 后，它才开始请求模型。这里的 `model` 是语言模型的 API 适配器，后面实验中的 AlexNet 则是待检查的神经网络。
...
例如，[我在 Mac 上的 AlexNet 实测记录](../examples/alexnet/checked-report.md)包含约 120 万训练样本、batch 128、约 90 轮，以及 MPS 单步检查通过、未提供数据目录。交给难度助手时，除了这些结论，还要带上论文页码和实验限制。你要填写自己这次的实测结果，设备和实验结果都以你的电脑为准。
...
你可以按自己的电脑选择 CUDA、MPS 或 CPU。我用 Mac 演示，你不必使用相同设备：
...
API、专家配置和工具函数都已提供。三个方法的消息格式和错误处理要求见[作业说明](assignment.md)。补全后，在仓库目录运行离线测试：
...
这些测试使用预设的模型回复，不需要密钥。测试通过后，安装 PyTorch，把 DeepSeek 密钥填进 `.env`，再让助手调查仓库中的 AlexNet 论文：

```bash
uv sync --locked --extra ml
# 如果还没有 .env，先从模板复制；已有配置则跳过这行
cp -n .env.example .env
# 打开 .env，填入 DEEPSEEK_API_KEY
uv run --extra ml python main.py --trace
```

`main.py` 默认读取 `examples/alexnet/paper.pdf`，并提供对应的单步实验。运行时看终端中的任务和工具结果，结束后打开 `output/report.md`，对照 PDF 和实验记录核查报告。
