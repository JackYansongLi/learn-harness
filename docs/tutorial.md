...
下面按一次调查的调用顺序讲解代码。完成末尾的作业后，就能按 [README](../README.md#以-alexnet-论文为例运行调查助手) 运行助手。
...
API、专家配置和工具函数都已提供，按[作业说明](assignment.md)中的消息格式与错误约定把它们接起来即可。写完先跑离线测试，再让助手调查 AlexNet 论文，检查真实 API 和工具调用是否正常，最后对照 PDF 和工具输出检查报告。这三步分别检查代码调用、真实执行和结论依据。
